...
As of `sharp@0.35.0`, building from source is no longer triggered automatically during `npm install`, so the layer build now installs the package first and then runs `sharp`'s build script against the custom `libvips` installation.
...
| upstream | 0.35.0 | 8.18.2 |  1.21.2 |   1.6.0 |    1.0.18 |    4.1 | 3.13.3 |     24 |
