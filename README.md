...
As of `sharp@0.35.3`, building from source is no longer triggered automatically during `npm install`, so the layer build now installs the package first and then runs `sharp`'s build script against the custom `libvips` installation.
...
| upstream | 0.35.3 | 8.18.3 |  1.23.0 |   1.6.0 |    1.0.18 |    4.1 | 3.14.1 |     24 |
